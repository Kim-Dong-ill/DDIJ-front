...
function Header() {
    ...
    useEffect(() => {
        const location =
            splitUrl?.length >0 ? splitUrl[1] : splitUrl[0];
        setCurrentFirstUrl(location);
    }, [locationHook]);
    console.log("현재경로는"+currentFirstUrl)
    switch(currentFirstUrl){
        case "appealwrite" : title= "자랑하개"; break;
        case "newcircle" : title= "어디가개"; break;
        case "circles" : title= "함께걷개"; break;
        case "userinfo" : title= "마이펫이지"; break;
        case "usermod" : title= "마이펫이지"; break;
        case "mypet" : title= "마이펫이지"; break;
        default : title = ""
    }
  return (
    <>
